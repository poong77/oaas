...
| 관점 | 설명 |
|:-|:-|
| **문제 해결** | 기존: 호텔리어가 "지금 영업 중인가?" 알 수 없음 + 연락처 코드 하드코딩 → 신규: 4곳(헤더·사이드바 2·푸터)에서 실시간 영업상태 1분 자동 갱신 + 어드민이 한 화면에서 운영시간·휴무·긴급전화·대표전화·이메일·ARS·Fax·웹 모두 편집 가능 |
...
| **핵심 가치** | CLAUDE.md 8번 원칙 "어드민 DB 편집 우선" 완전 구현. 호텔 상황 변화(설 단축영업, 사옥 점검 임시휴무)를 어드민이 즉시 반영 → 고객 신뢰 ↑, 전화 폭주 및 응대 지연 제거 |
...
| Q | A |
|:-|:-|
| **호텔리어가 "운영 중"인데 왜 접수 마감?** | `intakeDeadline` 필드 확인. 영업 종료 전이라도 접수 마감 시간이 지나면 "접수 마감" 표시. `/admin/master/business-hours` 탭 ① 에서 `접수 마감` 시간 수정 |
...
| **영업시간 외 긴급전화 번호 변경** | 탭 ① "영업시간 외 긴급전화" 섹션 → 번호 + 안내문구 수정 |
