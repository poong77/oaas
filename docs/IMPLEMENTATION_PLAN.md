...
> `/admin/insights/messaging` — 호텔·호텔리어에게 메일(SES) 또는 문자(Solapi)를 매니저/어드민이 직접 발송. 메일/문자/메시지함 3탭. 발송 이력은 `notification_logs`를 `batch_id`로 묶어 메시지함에 노출. **자동 알림(IS-03)·수동 티켓 발송(IS-06)과 구분되는 "캠페인성 일괄 발송" 도구**.

#### 현재 구현 (2026-06, 운영 중)

| 영역 | 구현 상태 |
|:-|:-|
| 메일 탭 | 발신자(`{local}@oapms.com`)·제목·본문(RichEditor)·변수칩·AI작성(Claude)·푸터 자동첨부 |
| 문자 탭 | 발신번호(고정)·제목(선택)·본문·바이트수·SMS/LMS/MMS 판정·예상비용 |
| 메시지함 탭 | 발송묶음 목록·검색(발송일·유형·업체·메일·문자번호)·페이지네이션(20/50/100)·모바일 카드뷰·수신자/본문 모달 |
| 수신자 선택 | 호텔 검색 → 직원(`users`)+추가연락처(`hotels.extra_contacts/extra_emails`) 조회, 칩 추가 |
| 변수 치환 | `#{업체명}`·`#{담당자명}`·`#{연락처}`·`#{호텔명}` 수신자별 치환 |
| 발송 | 순차 루프(최대 200명), `notification_logs`(eventKey `manual.email`/`manual.sms`, `batch_id`) + `activity_logs` 기록 |

#### 개선 백로그 (2026-06-16 도출, 조사 근거 `messaging-actions.ts`·`messaging-client.tsx`)

| ID | 개선 기능 | 핵심 동작 | 현재 문제 | 우선순위 |
|:-:|:-|:-|:-|:-:|
| MSG-01 | 부분 실패 가시화 | 발송 결과를 수신자별 성공/실패로 메시지함에 표시. 실패 사유 노출 | 순차 루프 중 실패해도 `{sent, failed}` 숫자만 반환, 누가 실패했는지 추적 불가 | P1 |
| MSG-02 | 발송 전 미리보기·확정 2단계 | 발송 직전 수신자별 변수치환 결과 미리보기. 미매핑 변수 경고 | `#{업체명}` 등 값 없으면 빈 문자열로 그대로 발송 | P1 |
| MSG-03 | 실패 건 재발송 | 메시지함에서 `status=failed` 수신자만 골라 재발송 | 실패 기록만 남고 재발송 동선 없음 | P1 |
| MSG-04 | 수신자 유효성 강화 | 전화번호 010 형식·이메일 강화 검증, 발송 전 차단 | 전화 9~11자리 길이만 검증 → 잘못된 번호 비용 소모 | P1 |
| MSG-05 | 수신거부(옵트아웃) 처리 | 광고성 문자 080 수신거부·야간(21~08시) 발송 제한·수신거부 명단 | 법적 안내 박스만 존재, 실제 처리 미구현 | P1 |
| MSG-06 | 예약 발송 | 발송 시각 지정 + 스케줄 큐 | 즉시 발송만 가능 | P2 |
| MSG-07 | 수동 발송 템플릿 저장/불러오기 | 자주 쓰는 문구를 저장하고 불러오기 | 마스터 `notification_templates`는 자동알림 전용, 수동발송 화면 연결 없음 | P2 |
| MSG-08 | MMS 이미지 첨부 | Solapi 이미지 업로드 실제 구현 | `classifySms`가 'mms' 반환하나 첨부 로직 부재(표시만) | P2 |
| MSG-09 | 메일 첨부파일 | 본문 외 파일 첨부 발송 | RichEditor 본문만 발송 가능 | P2 |
| MSG-10 | 수신자 그룹/세그먼트 저장 | "이 호텔군"을 그룹으로 저장·재사용 | 매번 호텔 검색→연락처 추가 반복 | P2 |
| MSG-11 | 발송 설정 마스터화 | 메일 도메인·기본 발신자(`as`)·SMS 단가(9/30원)·푸터 회사정보를 어드민 DB 편집으로 | 코드 하드코딩(`MAIL_DOMAIN`·`unitCost`·`format.ts`) — CLAUDE.md "어드민 DB 편집 우선" 원칙 위배 | P3 |
| MSG-12 | 발송 비용·통계 집계 | 메시지함에 누적 비용·기간별 발송량 집계 | 건수만 표시, 비용·추세 없음 | P3 |
| MSG-13 | batch msgType 정확도 | 수신자별 SMS/LMS 혼재 시 정확 분류 | 배치 첫 행 기준 단일 유형만 표시 | P3 |
| MSG-14 | 발송 결과 운영 알림 | 발송 완료/실패를 Slack으로 운영팀 통지 | 통지 훅 없음 | P3 |

> **권장 처리 순서**: MSG-01·02·03 묶음(부분실패 가시화 + 미리보기 + 실패 재발송)이 "200명 일괄 발송 중간 실패 시 복구 불가" 리스크를 직접 해소하므로 최우선. MSG-04·05는 발송 직전 차단/규제 대응이라 함께 묶어 진행 권장.

#### 필수 추가 (2026-06-16 사용자 지정 — 차기 작업 확정분)

> 아래 항목은 사용자가 **필수**로 지정. 우선 반영 대상. 적용 탭 = 메일/문자/공통 구분.

| ID | 적용 | 개선 기능 | 핵심 동작 |
|:-:|:-:|:-|:-|
| MSG-15 | 공통 | **변수 값 입력 필드 버그 수정** | 변수칩 선택 시 **값 입력 필드가 떠야 함**. 현재 누락으로 변수 기능 사실상 사용 불가 (**버그, 최우선**) |
| MSG-16 | 공통 | **같은 페이지 내 템플릿 탭 신설** | 자주 쓰는 템플릿 등록 → 발송 시 **수신정보·변수값만 교체**해 발송. 리스트: 문자/메일 구분 배지·제목·메모(요약)·수정/삭제(**아이콘만**)·**드래그앤드롭 정렬**. 편집은 **팝업 윈도우 에디터**(문자/메일 탭의 RichEditor·변수칩·편의기능 재사용) + 변수명 추가. ※ 기존 MSG-07을 본 항목으로 **구체화·대체** |
| MSG-17 | 공통 | **연락처 엑셀 업로드** | 엑셀 파일로 수신자 일괄 등록. 컬럼: **업체명 · 연락처(필수, 이메일 또는 휴대폰) · 변수명1~7**. 업로드 → 수신자 리스트 채움 |
| MSG-18 | 공통 | **호텔리어 전체 불러오기** | 호텔리어 계정 전체를 수신자로 일괄 추가 (**이메일 또는 문자** 채널 선택) |
| MSG-19 | 문자 | **제목 필수화 + 기본값** | 문자 제목을 **필수값**으로 변경, 기본 입력값 `[오아테크]` |
| MSG-20 | 메일 | **발신자명 입력창 신설** | 발신자 주소와 **같은 줄 좌측**에 발신자명(From display name) 입력창 추가, 기본값 `오아테크` |
| MSG-21 | 공통 | **발송 미리보기** | 발송 전 변수 치환이 적용된 실제 메시지(문자/메일) 미리보기. 문자는 말풍선 형태·바이트·유형 표시, 수신자 샘플 1명 기준 |
| MSG-22 | 공통 | **테스트 발송** | 지정한 테스트 번호(문자)/이메일로 1건 시험 발송. 본 발송 전 실물 확인. 이력은 테스트 표식 |
| MSG-23 | 메시지함 | **리스트 컬럼 점검 개편** | `총발송·성공·실패` 3칸 → `결과`(성공/실패) 1칸 통합, 그 자리에 `수신`(업체 요약·N명) 노출. 컬럼: 발송일시·유형·제목·수신·결과·발송자 |

> **연계 정리**: MSG-16은 기존 MSG-07(템플릿 저장/불러오기)을 흡수해 구체화 — MSG-07은 MSG-16으로 통합. MSG-17·18은 수신자 입력 경로 확장으로, 기존 MSG-10(수신자 그룹 저장)과 별개(MSG-10은 저장형, 17·18은 즉시 불러오기형). MSG-15는 변수 기능 자체를 살리는 선결 버그라 MSG-02(변수 미리보기)보다 먼저 처리.
