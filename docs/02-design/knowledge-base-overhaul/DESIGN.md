...
## 16. A6 재편집 세부 설계 (v1.2 + v1.4 모델 분기)

### 16-0. 모델 분기 (v1.4 결정)

| 호출 | 모델 | 이유 | 호출당 추정 비용 |
|---|---|---|---|
| **A5 메타 추출** | `claude-sonnet-4-6` | 5종 JSON 동시, 한국어 CS 톤, JSON 스키마 엄격 | ~$0.012 |
| **A6-1 reorder** (의도 변경) | `claude-sonnet-4-6` | 본문 구조 보존 + 골격 재배치 | ~$0.025 |
| **A6-2 fill-gaps** (빈 H2 채우기) | `claude-sonnet-4-6` | 컨텍스트 추론 필요 | ~$0.015 |
| **A6-3 tone** (CS 톤 보정) | **`claude-haiku-4-5-20251001`** ✅ | 단순 표현 변환, 구조 유지, JSON 스키마 단순 — 60% 비용↓ | ~$0.005 |
| **A6-4 custom** (사용자 명령) | `claude-sonnet-4-6` | 사용자 의도 다양, 안전 우선 | ~$0.020 |

**월 비용 추정** (매니저 5명 · 일 20회 호출 가정):
- A5(1500회) Sonnet ≈ $18
- A6(1500회) 혼합 = A6-1·2·4 Sonnet 1200회 × $0.020 + A6-3 Haiku 300회 × $0.005 ≈ $25
- **월 합계 ≈ $43** (예산 $50 안)

**v1.5 prompt cache 확장** (~~5000자+ system 프롬프트~~):
- 호텔리어 어휘 사전 + content_type 예시 추가
- cache hit 시 input 토큰 90% 절감 → 월 비용 ~$28 추정
