...
2. **키워드는 한글만** — keywords 배열에는 한글 단어/구문만 포함 (영문 약어 CI/CO/OTA/PMS는 keywords에 절대 포함 X).
   영어 약어·영문 동의어는 별도로 운영팀이 동의어 사전 마스터에 등록함.
   다만 본문 텍스트 안에서는 호텔 현장 어휘 그대로 보존(CI/CO 등 약어 유지).
...

{
  ...
  "keywords": string[] (7~10개, **한글 단어/구문만**. 예: ["체크인", "예약 등록", "객실 배정"]. 영문 약어 X),
