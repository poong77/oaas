...
// client-safe 부분(types/enum/labels/zod schema)은 별도 파일로 분리되어 있음.
// 이 파일은 server-only — system 프롬프트와 SDK 모델 분기를 포함.
// 호환을 위해 동일 export 경로를 유지 (기존 server 코드는 이 파일에서 계속 import 가능).
export {
  type RewriteMode,
  REWRITE_MODES,
  REWRITE_MODE_LABEL,
  ...
  type RewriteOutput,
  RewriteOutputSchema,
} from './article-rewriter-types';
import type { RewriteMode, RewriteInput } from './article-rewriter-types';
