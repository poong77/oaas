 ...
 * 출력: 마크다운 본문 초안 텍스트 (## H2 섹션 구조) — JSON 래핑 없이 평문 반환.
 *       긴 멀티라인 마크다운을 JSON 문자열로 감싸면 이스케이프가 깨지기 쉬워 평문 사용.
 *
 * 원칙:
...
const COMMON_HEADER = `당신은 호텔 OA 솔루션(PMS/CMS/Keyless/키오스크/웹) OA서포트 포털의 공지 본문 초안 작성 보조입니다.
...
- 마크다운 본문 초안 텍스트만 출력한다. \`## 섹션\` H2 구조를 사용한다.
- JSON·코드펜스(\`\`\`)·머리말 인사("아래는 …")·꼬리말 설명을 절대 붙이지 않는다.
- 첫 줄부터 바로 본문(보통 첫 \`## \` 헤더)으로 시작한다.
`;
...
/**
 * 모델이 실수로 감싼 코드펜스/머리말을 정리해 순수 마크다운 본문만 남김.
 *
 * - 전체를 감싼 ```/```markdown 펜스 제거
 * - 앞부분의 "아래는 …", "초안:" 류 인사말 1줄 제거(헤더/내용 전이면 유지)
 */
